...
### The API extension (implemented)

```c
/* Per-event metadata delivered to the registered callback. */
typedef struct {
    ...
    uint64_t sample_index;        /* bytes_out/2 at the boundary */
} rx888_pps_event_t;
...
The callback fires for both real PPS-induced short transfers and
for synthetic shorts generated by `cfg.debug_synthetic_pps_every`.
gr-rx888 and other consumers can therefore be developed end-to-end —
including the rx_time stream-tag plumbing — against stock firmware,
with no PPS hardware connected.

`sample_index` is captured *after* the closing transfer's samples
have been handed to the sample callback, so it points at the first
sample beyond the PPS edge — exactly the offset a GNURadio source
block needs when emitting an `rx_time` tag for the UTC second the
PPS edge represents.
